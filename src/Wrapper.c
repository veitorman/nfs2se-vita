...
#else
	const char *homeDir = "ux0:data/nfs2-recomp";
#endif
	if (homeDir && *homeDir)
	{
...
REALIGN STDCALL SDL_Window *WrapperCreateWindow(WindowProc windowProc)
{
	...
	sdlWin = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 960, 544, windowFlags);
	if (!sdlWin)
	{
