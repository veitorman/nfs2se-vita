...
void exit_func();
	...
	#include <sys/stat.h>
	/* Serial ports are unavailable on Vita. */
	#include <unistd.h>
	#include <fcntl.h>
	#include <errno.h>
...
REALIGN STDCALL void *CreateThread_wrap(void *threadAttributes, uint32_t stackSize, THREAD_START_ROUTINE startAddress, void *parameter, uint32_t creationFlags, uint32_t *threadId)
{
	...
	SDL_Thread *sdl_thread = SDL_CreateThreadWithStackSize(threadFunction, NULL, 1024*1024, thread);
	if (threadId)
...
REALIGN STDCALL File *CreateFileA_wrap(const char *fileName, uint32_t desiredAccess, uint32_t shareMode, SECURITY_ATTRIBUTES *securityAttributes, uint32_t creationDisposition, uint32_t flagsAndAttributes, void *templateFile)
{
	...
	switch (desiredAccess)
	{
		case GENERIC_READ | GENERIC_WRITE:
			if (COM_number)
				fd = -1; errno=ENOSYS; /* No serial device on Vita. */
			break;
		case GENERIC_WRITE:
			fd = open(tmpFileName, O_CREAT | O_WRONLY | O_TRUNC, 0644);
			break;
		case GENERIC_READ:
			if(!strcasecmp(tmpFileName,"install.win") || !strcasecmp(tmpFileName,"./install.win")) {
                vita_log("Game opening install03.win language override");
                fd = open("ux0:data/nfs2-recomp/install03.win", O_RDONLY);
            } else fd = open(tmpFileName, O_RDONLY);
			break;
	}
	if (fd > 0)
	{
...
REALIGN STDCALL void GetSystemInfo_wrap(SYSTEM_INFO *systemInfo)
{
	...
	/* Vita newlib does not export getpagesize; guest virtual pages are 4 KiB. */
	systemInfo->pageSize = 4096;
}
...
REALIGN STDCALL BOOL FindNextFileA_wrap(FindFile *findFile, WIN32_FIND_DATA *findFileData)
{
	...
	while ((de = readdir(findFile->dir)))
	{
		...
		if (*de->d_name == '.' || stat(de->d_name,&entryStat)!=0 || S_ISDIR(entryStat.st_mode))
			continue;
