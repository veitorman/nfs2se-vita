...
/* Native Cg shaders: explicit matching varyings; compile before linking.
 * uMatrix holds four explicit columns to avoid GLSL/Cg matrix-layout ambiguity. */
const char g_vShaderSrc[] =
"void main(float4 aPosition : POSITION, float4 aTexCoord : TEXCOORD0,\n"
" float4 aColor : COLOR0, float aFog : TEXCOORD1, uniform float4 uMatrix[4],\n"
" out float4 position : POSITION, out float4 vTexCoord : TEXCOORD0,\n"
" out float4 vColor : COLOR0, out float vFog : TEXCOORD1) {\n"
" position = aPosition.x*uMatrix[0] + aPosition.y*uMatrix[1] + aPosition.z*uMatrix[2] + aPosition.w*uMatrix[3];\n"
" vTexCoord = aTexCoord; vColor = aColor; vFog = aFog;\n"
"}\n";
const char g_fShaderSrc[] =
"float4 main(float4 vTexCoord : TEXCOORD0, float4 vColor : COLOR0,\n"
" float vFog : TEXCOORD1, uniform sampler2D uTextureSampler,\n"
" uniform float uTextureEnabled, uniform float uFogEnabled,\n"
" uniform float3 uFogColor) : COLOR {\n"
" float4 texel = uTextureEnabled * tex2D(uTextureSampler, vTexCoord.xy / vTexCoord.w);\n"
" float4 ret = vColor * ((1.0 - uTextureEnabled) + texel);\n"
" if (ret.a <= 16.0 / 255.0) discard;\n"
" ret.rgb = lerp(uFogColor, ret.rgb, 1.0 - (1.0 - vFog) * uFogEnabled);\n"
" return ret;\n"
"}\n";
const char g_vShaderDispSrc[] =
"void main(float2 aPosition : POSITION, float2 aTexCoord : TEXCOORD0,\n"
" out float4 position : POSITION, out float2 vTexCoord : TEXCOORD0) {\n"
" position = float4(aPosition, 0.0, 1.0); vTexCoord = aTexCoord;\n"
"}\n";
const char g_fShaderDispSrc[] =
"float4 main(float2 vTexCoord : TEXCOORD0, uniform sampler2D uTextureSampler,\n"
" uniform float uGamma) : COLOR {\n"
" float4 texel = tex2D(uTextureSampler, vTexCoord);\n"
" texel.rgb = pow(texel.rgb, float3(1.0 / uGamma));\n"
" return texel;\n"
"}\n";
...
{
	vita_log("Loading native Cg shaders (ZAPS04)");
	{
		...
		g_vShader = glCreateShader(GL_CG_VERTEX_SHADER_EXT);
		g_fShader = glCreateShader(GL_CG_FRAGMENT_SHADER_EXT);

		glShaderSource(g_vShader, 1, &vShaderSrc, &vShaderLen);
		glShaderSource(g_fShader, 1, &fShaderSrc, &fShaderLen);

		vita_log("Compiling game vertex");
		glCompileShader(g_vShader);
		if (!checkShaderCompilation(g_vShader))
			return false;
		vita_log("Compiling game fragment");
...

	{
		...
		g_vShaderDisp = glCreateShader(GL_CG_VERTEX_SHADER_EXT);
		g_fShaderDisp = glCreateShader(GL_CG_FRAGMENT_SHADER_EXT);

		glShaderSource(g_vShaderDisp, 1, &vShaderSrc, &vShaderLen);
		glShaderSource(g_fShaderDisp, 1, &fShaderSrc, &fShaderLen);

		vita_log("Compiling display vertex");
		glCompileShader(g_vShaderDisp);
		if (!checkShaderCompilation(g_vShaderDisp))
			return false;
		vita_log("Compiling display fragment");
		...
		vita_log("Linking display program");
...
static void drawFpsOverlay(void){
 if(!vita_config.fps)return;
 static unsigned frames,last,value;unsigned now=SDL_GetTicks();++frames;
 int changed=0;if(!last)last=now;if(now-last>=500){value=frames*10000/(now-last);frames=0;last=now;changed=1;}
 if(!fps_texture){glGenTextures(1,&fps_texture);changed=1;}
 glBindTexture(GL_TEXTURE_2D,fps_texture);
 if(changed){
  static unsigned pixels[128*16];for(int i=0;i<128*16;i++)pixels[i]=0xff101010;
  char label[24];snprintf(label,sizeof(label),"FPS %u.%u",value/10,value%10);
  for(int i=0;label[i]&&i<20;i++)for(int y=0;y<7;y++)for(int x=0;x<5;x++)if(font[(unsigned char)label[i]][y]&(1<<(4-x)))pixels[(y+4)*128+4+i*6+x]=0xffffffff;
  glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,128,16,0,GL_RGBA,GL_UNSIGNED_BYTE,pixels);
  glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
 }
 static const float uv[4][2]={{0,0},{0,1},{1,0},{1,1}};
 glViewport(8,504,256,32);glScissor(8,504,256,32);
 glUniform1f(g_uGammaLocDisp,1.0f);glVertexAttribPointer(g_aTexCoordLocDisp,2,GL_FLOAT,GL_FALSE,0,uv);
 glDrawArrays(GL_TRIANGLE_STRIP,0,4);glVertexAttribPointer(g_aTexCoordLocDisp,2,GL_FLOAT,GL_FALSE,0,g_textureCoordDisp);
 glUniform1f(g_uGammaLocDisp,g_gammaValue);glBindTexture(GL_TEXTURE_2D,0);
}
...
static void createFrameBuffer()
{
	...
	{
		vita_log("STOP: framebuffer incomplete");
		framebufferError = true;
		vita_stop("STOP: framebuffer unavailable");
	}

	vita_log("Framebuffer ready");
...
static void createContext()
{
	...
	if (vSync >= 0)
		vglWaitVblankStart(vSync != 0);

	glEnable(GL_SCISSOR_TEST);
	/* No dithering state in vitaGL. */

	glDepthFunc(GL_LEQUAL);

	if (!loadShaders())
	{
		vita_log("STOP: shader compilation/link failed");
		shaderError = true;
		vita_stop("STOP: shaders unavailable; see compiler log above");
	}
}
static void destroyContext()
{
 if(fps_texture){glDeleteTextures(1,&fps_texture);fps_texture=0;}
	uint32_t i;
	...
	/* vitaGL survives until process exit. */
	g_glCtx = NULL;
...
static inline void drawTriangles()
{
	...
	vita_draw_count(g_trianglesCount);
...
// 	fprintf(stderr, "grClipWindow: %d %d %d %d [%d]\n", minX, minY, maxX, maxY, g_trianglesCount);
	...
	glUniform4fv(g_uMatrixLoc, 4, g_matrix);
}
